...
interface NseBb30TabContentProps {
  autoRefresh?: boolean;
}
...
const SortDropdown: React.FC<{
  ...
  <div className="absolute top-full right-0 mt-1 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl z-30 py-1 min-w-[180px] animate-slideDown">
    {SORT_OPTIONS.map(({ key, label }) => (
      ...
      >
        {label}
        {current === key && <Check className="w-3.5 h-3.5 inline ml-2 text-sky-400" />}
      </button>
    ))}
  </div>
);
...
const EmptyState: React.FC<{ hasFilters: boolean; onReset: () => void }> = ({ hasFilters, onReset }) => (
  <div className="flex flex-col items-center justify-center py-20 text-center px-6">
    ...
    <p className="text-slate-500 text-sm mb-5 max-w-xs">
      {hasFilters
        ? 'No NSE-BB-30-30 signals match your current filters. Try adjusting your criteria.'
...
/* ═══════════════════════════════════════════════════════════════
   NSE-BB-30 TRADING CARD — Same layout as FUKAA
   ═══════════════════════════════════════════════════════════════ */
      ...
      <div className="p-3 sm:p-4">
        ...
        <div className="mt-2.5 space-y-1.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className={`text-sm font-bold font-mono w-7 text-right ${confidence >= 75 ? 'text-sky-400' : confidence >= 55 ? 'text-green-400' : confidence >= 35 ? 'text-amber-400' : 'text-red-400'}`}>
              {confidence}
            </span>
          </div>
        </div>
        ...
        {/* -- FUNDED TRADE BADGE -- */}
        {isFunded && (
          <div className="mt-2 flex items-center gap-1.5 px-2 py-1 rounded-lg bg-sky-500/10 border border-sky-500/25">
            <span className="text-sky-400 font-bold text-sm">₹</span>
            <span className="text-[10px] text-sky-400">Trade executed with added funds</span>
          </div>
        )}
...
/* ═══════════════════════════════════════════════════════════════
   MAIN: NSE-BB-30 TAB CONTENT
   ═══════════════════════════════════════════════════════════════ */

export const NseBb30TabContent: React.FC<NseBb30TabContentProps> = ({ autoRefresh = true }) => {
  ...
  const fetchTriggers = useCallback(async () => {
    try {
      const data = await fetchJson<McxBbTrigger[]>('/strategy-state/nsebb30/triggers');
      if (Array.isArray(data)) {
        setTriggers(data);
      }
    } catch (err) {
      console.error('Error fetching NSE-BB-30 triggers:', err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    const fetchCapital = async () => {
      try {
        const data = await strategyWalletsApi.getCapital('NSE_BB_30');
    ...
    const onWalletUpdate = (e: Event) => {
      ...
      if (detail?.strategy === 'NSE_BB_30' || detail?.strategy === 'NSE-BB-30') fetchCapital();
    };
      ...
      <div className="sticky top-0 z-20 bg-slate-900/95 backdrop-blur-md border-b border-slate-700/50">
        <div className="flex items-center justify-between h-14 px-2 sm:px-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-semibold text-sky-400 tracking-tight">NSE-BB-30</h1>
          </div>
      ...
      <div className="pt-4 pb-8">
        ...
        {loading && triggers.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <span className="text-slate-500 text-sm">Loading NSE-BB-30-30 signals...</span>
          </div>
        )}
            ...
            {sorted.map(({ sig, plan }) => (
              <McxBbCard
                ...
                onRequestFunds={(s, p, credit, prem, lotSz, mult, conf) => {
                  setFundModal({ strategyKey: 'NSE_BB_30', creditAmount: credit, sig: s, plan: p, premium: prem, lotSize: lotSz, multiplier: mult, confidence: conf });
                }}
...
export default NseBb30TabContent;
