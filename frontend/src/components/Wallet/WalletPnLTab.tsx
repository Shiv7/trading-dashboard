  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* By Strategy */}
        <div className="bg-slate-800/50 backdrop-blur border border-slate-700/50 rounded-xl p-6">
          <h3 className="text-lg font-semibold text-white mb-4">By Strategy</h3>
          {strategies.length > 0 ? (
                ...
                return (
                  <div key={strat.strategy} className="flex items-center gap-3">
                    ...
                    <div className="text-right w-28">
                      <span className={`text-sm font-mono tabular-nums ${strat.totalPnl >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                        {formatCurrency(strat.totalPnl)}
                      </span>
                      <span className="text-xs text-slate-500 ml-2">{strat.trades}t</span>
                    </div>
                  </div>
                )
              })}
        ...
        {/* By Instrument */}
        <div className="bg-slate-800/50 backdrop-blur border border-slate-700/50 rounded-xl p-6">
          <h3 className="text-lg font-semibold text-white mb-4">By Instrument</h3>
              ...
              {instruments.slice(0, 10).map(inst => {
                ...
                return (
                  <div key={inst.scripCode} className="flex items-center gap-3">
                    ...
                    <div className="text-right w-28">
                      <span className={`text-sm font-mono tabular-nums ${inst.totalPnl >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                        {formatCurrency(inst.totalPnl)}
                      </span>
                      <span className="text-xs text-slate-500 ml-2">{inst.winRate?.toFixed(0)}%</span>
                    </div>
                  </div>
                )
              })}
      ...
      {metrics && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-4">
            ...
            <p className="text-lg font-bold font-mono text-emerald-400">{formatCurrency(metrics.avgWin)}</p>
          </div>
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-4">
            <p className="text-xs text-slate-400 mb-1">Avg Loss</p>
            <p className="text-lg font-bold font-mono text-red-400">{formatCurrency(metrics.avgLoss)}</p>
          </div>
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-4">
            <p className="text-xs text-slate-400 mb-1">Best Trade</p>
            <p className="text-lg font-bold font-mono text-emerald-400">{formatCurrency(metrics.bestTrade)}</p>
          </div>
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-4">
            <p className="text-xs text-slate-400 mb-1">Worst Trade</p>
            <p className="text-lg font-bold font-mono text-red-400">{formatCurrency(metrics.worstTrade)}</p>
          </div>
        </div>
      )}
    </div>
