...
/**
 * Kafka consumer for MCX-BB-15 signals (MCX Bollinger Band 15-min breakout strategy).
 * Displays signals on dashboard and broadcasts via WebSocket.
 */
@Component
@Slf4j
public class McxBb15Consumer implements OptionSwapAware {
    ...
    @KafkaListener(
            topics = {"kotsin_MCX_BB_15"},
            groupId = "${spring.kafka.consumer.group-id:trading-dashboard-v2}"
    )
    public void onMcxBb15(String payload) {
        try {
            ...
            log.info("MCX-BB-15 TRIGGER: {} ({}) {} @ {} | surge={}x oiChg={}% score={}",
            ...
            data.put("signalSource", "MCX_BB_15");
            ...
            latestMcxBb15.put(scripCode, data);

            // Register as signal on dashboard
            String rationale = String.format("MCX-BB-15: %s BB breakout | surge=%.1fx oiChg=%.1f%%",
            ...
            SignalDTO signalDTO = SignalDTO.builder()
                    ...
                    .signalSource("MCX_BB_15")
                    .signalSourceLabel("MCX-BB-15")
                    .signalType("MCX_BB_15")
                    .direction(direction)
            ...
            sessionManager.broadcastNotification("MCX_BB_15_TRIGGER",
                    String.format("%s %s MCX-BB-15 for %s @ %.2f | surge=%.1fx oiChg=%.1f%%",
                            emoji, direction, displayName, triggerPrice, surgeT, oiChangeRatio));

        } catch (Exception e) {
            log.error("Error processing MCX-BB-15: {}", e.getMessage(), e);
        }
    }
    ...
    public Map<String, Map<String, Object>> getLatestMcxBb15() {
        return Collections.unmodifiableMap(latestMcxBb15);
    }
    ...
    public void updateTradedOption(String underlyingScripCode, String strategy,
        ...
        if (!"MCX_BB_15".equals(strategy)) return;

        Map<String, Object> signal = latestMcxBb15.get(underlyingScripCode);
        if (signal != null) {
            ...
            log.info("MCX_BB_15 option swap applied to cached signal: scrip={} -> {}@{} strike={}",
