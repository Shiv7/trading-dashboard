...
/**
 * Kafka consumer for MCX-BB-30 signals (MCX Bollinger Band 30-min breakout strategy).
 * Displays signals on dashboard and broadcasts via WebSocket.
 */
@Component
@Slf4j
public class McxBb30Consumer implements OptionSwapAware {
    ...
    @Value("${mcxbb30.signal.ttl.minutes:30}")
    ...
    @KafkaListener(
            topics = {"kotsin_MCX_BB_30"},
            groupId = "${spring.kafka.consumer.group-id:trading-dashboard-v2}"
    )
    public void onMcxBb30(String payload) {
        try {
            ...
            log.info("MCX-BB-30 TRIGGER: {} ({}) {} @ {} | surge={}x oiChg={}% score={}",
            ...
            data.put("signalSource", "MCX_BB_30");
            ...
            latestMcxBb30.put(scripCode, data);

            // Register as signal on dashboard
            String rationale = String.format("MCX-BB-30: %s BB breakout | surge=%.1fx oiChg=%.1f%%",
            ...
            SignalDTO signalDTO = SignalDTO.builder()
                    ...
                    .signalSource("MCX_BB_30")
                    .signalSourceLabel("MCX-BB-30")
                    .signalType("MCX_BB_30")
                    .direction(direction)
            ...
            sessionManager.broadcastNotification("MCX_BB_30_TRIGGER",
                    String.format("%s %s MCX-BB-30 for %s @ %.2f | surge=%.1fx oiChg=%.1f%%",
    ...
    public Map<String, Map<String, Object>> getLatestMcxBb30() {
        return Collections.unmodifiableMap(latestMcxBb30);
    }
    ...
    public void updateTradedOption(String underlyingScripCode, String strategy,
        ...
        if (!"MCX_BB_30".equals(strategy)) return;

        Map<String, Object> signal = latestMcxBb30.get(underlyingScripCode);
        if (signal != null) {
            ...
            log.info("MCX_BB_30 option swap applied to cached signal: scrip={} -> {}@{} strike={}",
