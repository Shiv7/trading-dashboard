...
public class FUKAAConsumer {
    ...
    // Cache latest FUKAA signals (no TTL - informational only)
    ...
    public void onFUKAA(String payload) {
        try {
            ...
            if (triggered) {
                ...
                fukaaData.put("cachedAt", Instant.now().toEpochMilli());
                ...
                String rationale = String.format("FUKAA: %s via %s | Rank=%.2f Score=%.2f | BB[%.2f-%.2f] ST=%.2f",
                        fukaaData.get("fukaaOutcome"), fukaaData.get("passedCandle"),
                        rank, triggerScore, bbLower, bbUpper, superTrend);
    ...
    public void persistToRedis() {
        try {
            ...
            log.info("FUKAA persisted {} active triggers to Redis", snapshot.size());
        } catch (Exception e) {
    ...
    private void restoreFromRedis() {
        try {
            ...
            if (entries == null || entries.isEmpty()) {
                log.info("FUKAA: No persisted triggers found in Redis");
                return;
            }
            int restored = 0;
            ...
            log.info("FUKAA restored {} active triggers from Redis (skipped stale)", restored);
        } catch (Exception e) {
    ...
    private void resetDailyCounterIfNeeded() {
        ...
        if (!today.equals(currentTradeDate)) {
            ...
            log.info("FUKAA daily signal counters reset for {}", today);
        }
    }
