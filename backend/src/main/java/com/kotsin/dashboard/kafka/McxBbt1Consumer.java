...
/**
 * Kafka consumer for MCX-BBT1 signals (MCX Bollinger Band T+1 breakout strategy).
 * Displays signals on dashboard and broadcasts via WebSocket.
 */
@Component
@Slf4j
public class McxBbt1Consumer implements OptionSwapAware {
    ...
    public void onMcxBbt1(String payload) {
        try {
            ...
            log.info("MCX-BBT1 TRIGGER: {} ({}) {} @ {} | surge={}x oiChg={}% score={}",
            ...
            data.put("signalSource", "MCX_BBT1");
            ...
            latestMcxBbt1.put(scripCode, data);

            // Register as signal on dashboard
            String rationale = String.format("MCX-BBT1: %s BB breakout | surge=%.1fx oiChg=%.1f%%",
            ...
            SignalDTO signalDTO = SignalDTO.builder()
                    ...
                    .signalSource("MCX_BBT1")
                    .signalSourceLabel("MCX-BBT+1")
                    .signalType("MCX_BBT1")
                    .direction(direction)
            ...
            sessionManager.broadcastNotification("MCX_BBT1_TRIGGER",
                    String.format("%s %s MCX-BBT1 for %s @ %.2f | surge=%.1fx oiChg=%.1f%%",
                            emoji, direction, displayName, triggerPrice, surgeT, oiChangeRatio));

        } catch (Exception e) {
            log.error("Error processing MCX-BBT1: {}", e.getMessage(), e);
        }
    }
    ...
    public Map<String, Map<String, Object>> getLatestMcxBbt1() {
        return Collections.unmodifiableMap(latestMcxBbt1);
    }
    ...
    public void updateTradedOption(String underlyingScripCode, String strategy,
        ...
        if (!"MCX_BBT1".equals(strategy)) return;

        Map<String, Object> signal = latestMcxBbt1.get(underlyingScripCode);
        if (signal != null) {
            ...
            log.info("MCX_BBT1 option swap applied to cached signal: scrip={} -> {}@{} strike={}",
